"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.DynamicFanoutError = void 0;exports.assertJsonPointer = assertJsonPointer;exports.assertNoUnresolvedItemReferences = assertNoUnresolvedItemReferences;exports.collectDynamicResults = collectDynamicResults;exports.hasDynamicFanoutFields = hasDynamicFanoutFields;exports.isSafeOutputName = isSafeOutputName;exports.materializeDynamicParallelStep = materializeDynamicParallelStep;exports.normalizeItemKeyForId = normalizeItemKeyForId;exports.resolveDynamicFanoutItems = resolveDynamicFanoutItems;exports.resolveItemTemplate = resolveItemTemplate;exports.resolveJsonPointer = resolveJsonPointer;exports.validateDynamicCollection = validateDynamicCollection;exports.validateDynamicStepShape = validateDynamicStepShape;

var _utils = await jitiImport("../../shared/utils.ts");
var _structuredOutput = await jitiImport("./structured-output.ts");

class DynamicFanoutError extends Error {}exports.DynamicFanoutError = DynamicFanoutError;
...
function assertJsonPointer(pointer, label) {
  if (pointer === "") return;
...
function normalizeItemKeyForId(key) {
...
function resolveItemTemplate(template, itemName, item) {
...
function assertNoUnresolvedItemReferences(template, itemName, label) {
...
function hasDynamicFanoutFields(step) {
...
function validateDynamicStepShape(step, stepIndex, config = {}) {
  ...
  if ("expand" in step.parallel) throw new DynamicFanoutError(`${prefix} does not support nested dynamic fanout.`);
...
function resolveDynamicFanoutItems(step, outputs, stepIndex, config = {}) {
...
function materializeDynamicParallelStep(step, outputs, stepIndex, config = {}) {
...
results)
{
  return items.map((entry, index) => {
    ...
    const text = result ?
    "output" in result && typeof result.output === "string" ? result.output : (0, _utils.getSingleResultOutput)(result) :
    "";
    return {
      key: entry.key,
      index: entry.index,
...
function validateDynamicCollection(schema, value) {
  if (!schema) return;
  const validation = (0, _structuredOutput.validateStructuredOutputValue)(schema, value);
