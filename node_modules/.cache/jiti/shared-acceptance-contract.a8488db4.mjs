"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.acceptanceSelfReviewConfig = acceptanceSelfReviewConfig;exports.formatAcceptancePrompt = formatAcceptancePrompt;exports.formatEvidenceReportFieldMapping = formatEvidenceReportFieldMapping;exports.resolveEffectiveAcceptance = resolveEffectiveAcceptance;exports.shouldRunAcceptanceFinalization = shouldRunAcceptanceFinalization;exports.validateAcceptanceInput = validateAcceptanceInput;
...
function formatEvidenceReportFieldMapping(evidence) {
      ...
      for (const [index, criterion] of value.criteria.entries()) {
        ...
        if (Array.isArray(item.evidence)) {
          for (const [evidenceIndex, evidence] of item.evidence.entries()) {
            if (typeof evidence !== "string" || !VALID_EVIDENCE.has(evidence)) {
    ...
    for (const [index, command] of value.verify.entries()) {
      ...
      if (cmd.env !== undefined) {
        if (!cmd.env || typeof cmd.env !== "object" || Array.isArray(cmd.env)) {
          ...
          for (const [key, envValue] of Object.entries(cmd.env)) {
...
function deriveAcceptanceLevel(config) {
...
function formatAcceptancePrompt(acceptance) {
